-- =============================================================================
-- Fix: "permission denied for table daily_demand_summary" when calling
--      analytics.refresh_daily_demand_summary() from the dashboard (authenticated).
-- =============================================================================
-- The function INSERTs into analytics.daily_demand_summary and runs
-- REFRESH MATERIALIZED VIEW on analytics.daily_demand_summary_product.
-- By default it runs as the invoker (authenticated), who has no rights on
-- those objects. SECURITY DEFINER makes it run with the function owner's
-- privileges. (PostgreSQL does not support ALTER FUNCTION SET SECURITY DEFINER;
-- the function must be recreated with SECURITY DEFINER in the definition.)
-- =============================================================================
-- Run this once in Supabase SQL Editor (as a user that can create/replace the function).
-- =============================================================================
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = analytics, ezorder, beeline_prod
AS $$
BEGIN
